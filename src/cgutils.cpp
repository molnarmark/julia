    ...
    if (jl_is_leaf_type(jt)) {
        if ((jl_is_bitstype(jt) || jl_isbits(jt))) {
            if (jl_datatype_nbits(jt) == 0)
                return T_void;
            Type *t = julia_struct_to_llvm(jt, isboxed);
            assert(t != NULL);
            return t;
        }
    }
...
// compute whether all subtypes of this type have the same layout
// (which is equivalent to asking whether the types of any of the
// fields depend on any of the unbound parameters of the type)
static bool julia_struct_has_layout(jl_datatype_t *dt)
{
    ...
    for (i = 0; i < ntypes; i++) {
        ...
        if (jl_has_typevars_from(ty, dt->parameters))
            return false;
    }
    return true;
}
...
static Value *init_bits_value(Value *newv, Value *v, MDNode *tbaa, unsigned alignment = sizeof(void*)) // min alignment in julia's gc is pointer-aligned
{
    ...
    tbaa_decorate(tbaa, builder.CreateAlignedStore(v, emit_bitcast(newv, PointerType::get(v->getType(),0)), alignment));
    return newv;
}
