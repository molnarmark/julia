...
// --- entry point ---
//static int n_emit=0;
static std::unique_ptr<Module> emit_function(jl_method_instance_t *lam, jl_code_info_t *src, size_t world, jl_llvm_functions_t *declarations);
...
extern "C"
jl_llvm_functions_t jl_compile_linfo(jl_method_instance_t **pli, jl_code_info_t *src, size_t world)
{
    JL_TIMING(CODEGEN);
    ...
    JL_TRY {
        ...
        if (li->min_world <= world && li->max_world >= world)
            ...
            pdecls = &decls;
        m = emit_function(li, src, world, pdecls);
        if (world)
            decls = li->functionObjectsDecls;
        //n_emit++;
    }
...
jl_generic_fptr_t jl_generate_fptr(jl_method_instance_t *li, void *_F, size_t world)
{
    ...
    if (li->def) {
        ...
        if (!F || !jl_can_finalize_function(F)) {
            ...
            assert(src);
            F = (Function*)jl_compile_linfo(&unspec, src, unspec->min_world).functionObject; // this does not change unspec
            if (unspec == li) {
...
extern "C" JL_DLLEXPORT
void *jl_get_llvmf_defn(jl_method_instance_t *linfo, size_t world, bool getwrapper)
{
    ...
    std::unique_ptr<Module> m;
    JL_TRY {
         m = emit_function(linfo, src, world, &declarations);
    }
    JL_CATCH {
        // something failed!
        nested_compile = last_n_c;
...
extern "C" JL_DLLEXPORT
void *jl_get_llvmf_decl(jl_method_instance_t *linfo, size_t world, bool getwrapper)
{
    // `source` is `NULL` for generated functions.
    // The `isstaged` check can be removed if that is not the case anymore.
    if (linfo->def && linfo->def->source == NULL && !linfo->def->isstaged) {
    ...
    if (decls.functionObject == NULL && linfo->jlcall_api == 2 && linfo->def) {
        ...
        if (decls.functionObject == NULL) {
            ...
            decls = jl_compile_linfo(&linfo, src, world);
...
static jl_cgval_t emit_invoke(jl_expr_t *ex, jl_codectx_t *ctx)
{
    ...
    if (lival.constant) {
        ...
        assert(jl_is_method_instance(li));
        jl_llvm_functions_t decls = jl_compile_linfo(&li, NULL, ctx->world);
        if (li->jlcall_api == 2) {
            assert(li->inferred);
                                  ...
                                  jl_typemap_entry_t *sf, jl_value_t *declrt, jl_tupletype_t *sigt)
{
    ...
    if (lam) {
        ...
        jl_compile_linfo(&lam, src, world);
        if (lam->jlcall_api != 2) {
