...
io = IOBuffer()
show(io, v)
str = String(takebuf(io))
show(io, v0)
@test str == String(takebuf(io))
show(io, A)
str = String(takebuf(io))
@test str == "[1 3; 2 4]"
show(io, S)
...
strs = split(strip(String(takebuf(io))), '\n')
@test strs[2] == " 1  3"
@test strs[3] == " 2  4"
...
function cmp_showf(printfunc, io, A)
    ...
    printfunc(ioc, A)
    str1 = String(takebuf(io))
    printfunc(ioc, parent(A))
    str2 = String(takebuf(io))
    @test str1 == str2
end
...
for n = 0:4
    ...
    @test String(takebuf(io)) == targets1[n+1]
    show(IOContext(io, limit=true), MIME("text/plain"), (a,a))
    @test String(takebuf(io)) == targets2[n+1]
end
