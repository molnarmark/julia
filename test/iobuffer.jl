...
@test String(takebuf(io)) == "boston\ncambridge\n"
@test String(takebuf(io)) == ""
...
@test_throws ArgumentError write(io,UInt8(0))
@test_throws ArgumentError write(io,UInt8[0])
@test String(takebuf(io)) == "hamster\nguinea pig\nturtle"
...
let io=IOBuffer(SubString("***αhelloworldω***",4,16)), io2 = IOBuffer(b"goodnightmoon", true, true)
    ...
    @test_throws ArgumentError write(io,"!")
    @test takebuf(io) == b"αhelloworldω"
    seek(io, 2)
    seekend(io2)
    write(io2, io)
    @test readstring(io) == ""
    @test readstring(io2) == ""
    @test String(takebuf(io)) == "αhelloworldω"
    seek(io2, 0)
